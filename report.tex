...
\begin{lstlisting}[language=bash]
137a6aa docs(report): include all 11 commits in git log listing + resync assets
...
互不合并，满足「禁止单次全量提交」。仓库已推送至 GitHub，线上真实提交记录见下方截图。
...
\caption{GitHub 仓库真实提交记录（github.com/yingjunzhu520/system-tools-hamesome-4，main 分支，共 13 次提交，最新提交 ecfb0a2）}
\label{fig:commit}
\end{figure}
...
仓库已完成推送并通过验证：
\begin{itemize}
  \item 远端地址：\url{https://github.com/yingjunzhu520/system-tools-hamesome-4}（main 分支）。
  \item 上图即推送后的 GitHub 仓库页面截图，显示 \textbf{13 次提交}，最新提交
        \texttt{ecfb0a2}，与本地 \texttt{git rev-parse HEAD} 一致。
  \item 服务端校验：\texttt{git ls-remote origin} 返回 \texttt{refs/heads/main}
        指向 \texttt{ecfb0a2}，与本地 HEAD 完全相同，确认全部提交已上线。
  \item 全程分次增量提交，\textbf{无单次全量提交}，符合课程要求。
\end{itemize}
